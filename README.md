...
### 2025.12.10 Baostock API 集成 - 数据源容错

我们新增了 **Baostock API** 作为备选数据源，解决了数据获取不稳定的问题：

1.  **双数据源容错机制**：当主要的 Akshare API 失败时，自动切换到 Baostock API
2.  **解决的问题**：
...
3.  **智能市值计算**：
    ...
    - 方法2：从每股收益（EPS）推算
    - 自动选择最佳方法
4.  **无需配置**：Baostock 完全免费，无需 API Key，自动激活
5.  **透明切换**：无需修改现有代码，系统自动处理 API 切换

**快速开始**: 查看 `QUICKSTART_BAOSTOCK.md` 了解更多信息。

**技术细节**: 查看 `BAOSTOCK_INTEGRATION.md` 和 `CHANGES.md` 了解完整实现。
